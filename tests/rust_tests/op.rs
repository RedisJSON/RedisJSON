...
#[test]
fn op_string_regexp_escape_match() {
    setup();

    select_and_then_compare(
        r#"$.arr[?(@ =~ "^(0|[1-9]\\d*)(\\.(0|[1-9]\\d*)){0,2}$")]"#, //regex
        json!({
            "arr": ["six.six", "1.2.3", "a1.2.3", "0x.2.3", "7.8", "5", "5.", "0.0.0", "00.0.0", "1.2.00", "2.42.2", "9", "9.8", "9.87.65", "9.87.65a", "1.2.3.4"],
        }),
        json!(["1.2.3", "7.8", "5", "0.0.0", "2.42.2", "9", "9.8", "9.87.65"]),
    );
}

#[test]
fn op_string_regexp_escape_field_match() {
    setup();
