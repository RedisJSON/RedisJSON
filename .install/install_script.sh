...
# ----------------------------------------------------------------------------
# RedisJSON: Rust via rustup + rust-toolchain.toml (glibc/macOS). Abstract
# `cargo` in dependencies.yaml is apk-only (Alpine musl); elsewhere packages are
# skipped so we install the pinned toolchain here — same entrypoint as Docker
# (`.install/getrust.sh`).
# ----------------------------------------------------------------------------
if [ "$OSNICK" != "alpine" ]; then
    if [ -x "$HERE/getrust.sh" ]; then
        echo "install_script.sh: RedisJSON — installing Rust (getrust.sh, cwd=$ROOT)"
        (cd "$ROOT" && bash "$HERE/getrust.sh" "$MODE")
    else
        echo "install_script.sh: error: getrust.sh missing (expected at $HERE/getrust.sh)" >&2
        exit 1
    fi
else
    echo "install_script.sh: skipping rustup on Alpine (distro cargo from dependencies.yaml package_map)"
fi
